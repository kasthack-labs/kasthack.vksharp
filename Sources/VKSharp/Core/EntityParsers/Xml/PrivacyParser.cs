using System;
using System.Collections.Generic;
using System.Linq;
using System.Xml;
using VKSharp.Core.Entities;
using VKSharp.Core.Interfaces;
using VKSharp.Data.Executors;
using VKSharp.Helpers;

namespace VKSharp.Core.EntityParsers.Xml {
    public class PrivacyParser : IXmlVKEntityParser<Privacy> {
        public IExecutor Executor { get; set; }

        private static Lazy<Dictionary<string, Action<Privacy, string>>> _generatedParsers =
    new Lazy<Dictionary<string, Action<Privacy, string>>>( ParserHelper.GetStringParsers<Privacy> );

        private static Dictionary<string, Action<Privacy, string>> GeneratedParsers {
            get {
                return _generatedParsers.Value;
            }
        }

        public void FillFromXml( IEnumerable<XmlNode> nodes, Privacy entity ) {
            foreach ( var cn in nodes )
                this.UpdateFromFragment( cn, entity );
        }

        public Privacy ParseFromXml( XmlNode node ) {
            return this.ParseFromXmlFragments( node.ChildNodes.OfType<XmlNode>() );
        }

        public Privacy[] ParseAllFromXml( IEnumerable<XmlNode> nodes ) {
            return nodes.Select( this.ParseFromXml ).Where( a => a != null ).ToArray();
        }

        public Privacy ParseFromXmlFragments( IEnumerable<XmlNode> nodes ) {
            var sc = new Privacy();
            this.FillFromXml( nodes, sc );
            return sc;
        }

        public bool UpdateFromFragment( XmlNode node, Privacy entity ) {
            Action<Privacy, string> parser;
            var nodeName = node.Name;
            if ( GeneratedParsers.TryGetValue( nodeName, out parser ) ) {
                parser( entity, node.InnerText );
                return true;
            }
            return false;
        }

    }
}
